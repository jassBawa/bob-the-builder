...
function IntegrityUSPVSecondFloorForm() {
  ...
  const handleLocationChange = (index, e) => {
    ...
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'location', newValue);
  };

  // Element
  const handleElementChange = (index, value) => {
    console.log(index, value);
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'element', value);
  };

  // Grade
  const handleGradeChange = (index, value) => {
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'grade', value);
  };

  // Grade Results
  const handleGradeResultsChange = (index, event) => {
    const newValue = event.target.value;
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'rhTestResults', newValue);
  };

  // Probing
  const handleProbingMethodChange = (index, value) => {
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'probingMethod', value);
  };

  // uspv
  const handleUSPVChange = (index, value) => {
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'upvValues', value);
    const grade = uspvData.second[index].grade;
    const metricGrade = getMetricGrade(grade, value);
    updateField('strucuturalIntegrity', 'USPV', 'second', index, 'metricGrade', metricGrade);
  };
...
export default IntegrityUSPVSecondFloorForm;
