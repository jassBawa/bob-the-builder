...
function USPVSecondFloorForm() {
  const { ndtdata, updateField } = useNdtStore();
  const uspvData = ndtdata.inSitu.USPV;

  const handleLocationChange = (index, e) => {
    const newValue = e.target.value;
    updateField('inSitu', 'USPV', 'second', index, 'location', newValue);
  };

  // Element
  const handleElementChange = (index, value) => {
    console.log(index, value);
    updateField('inSitu', 'USPV', 'second', index, 'element', value);
  };

  // Grade
  const handleGradeChange = (index, value) => {
    updateField('inSitu', 'USPV', 'second', index, 'grade', value);
  };

  // Grade Results
  const handleGradeResultsChange = (index, event) => {
    const newValue = event.target.value;
    updateField('inSitu', 'USPV', 'second', index, 'rhTestResults', newValue);
  };

  // Probing
  const handleProbingMethodChange = (index, value) => {
    updateField('inSitu', 'USPV', 'second', index, 'probingMethod', value);
  };

  // uspv
  const handleUSPVChange = (index, value) => {
    updateField('inSitu', 'USPV', 'second', index, 'upvValues', value);
    const grade = uspvData.second[index].grade;
    const metricGrade = getMetricGrade(grade, value);
    updateField('inSitu', 'USPV', 'second', index, 'metricGrade', metricGrade);
  };
...
export default USPVSecondFloorForm;
